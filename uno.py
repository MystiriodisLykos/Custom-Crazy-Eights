...
class UnoGame(object):
    ...
    values = [str(x) for x in range(1, 10)] + ['skip'] + ['+2'] + ['reverse']

    def __init__(self):
        self.players = {}
        self.cards = shuffle([[Card(c, v) for v in UnoGame.values]
                              for c in UnoGame.colors]*2 +
                             [[Card(c, '0') for c in UnoGame.colors]] +
                             [[Card('wild', v) for v in ['', '+4']]])
